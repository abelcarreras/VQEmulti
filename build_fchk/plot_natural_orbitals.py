...
r = 1.0
alpha = np.deg2rad(104.5)
h2o_molecule = MolecularData(geometry=[['O', [0, 0, 0]],
                                       ['H', [-r, 0, 0]],
...
mo_coeff = h2_molecule.canonical_orbitals

#print(mo_coeff)
#exit()
mo_energies = h2_molecule.orbital_energies # / 27.211407953
print('energies: ', mo_energies)

for e in mo_energies:
    print('{:5.3}'.format(e))

#can_orb = molecule._pyscf_data['scf'].mo_coeff

overlap_matrix = molecule._pyscf_data['scf'].get_ovlp(molecule._pyscf_data['mol'])
density = molecule._pyscf_data['scf'].make_rdm1()


fchk_txt = get_fchk_from_pyscf(molecule._pyscf_data['mol'], mo_coeff.T, mo_energies, density)
with open('pyscf_calc.fchk', 'w') as f:
    f.write(fchk_txt)