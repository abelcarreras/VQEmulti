...
def measureExpectation(mainString, subHamiltonian, shots, statePreparationGates, qubits):
    ...
    for j in range(shots):
        ...
        for i in range(n_qubits):
            ...
            if mainString[i] != "I":
                ...
                for subString in subHamiltonian:
                    #print(results.data[str(i)][j], (1 - 2 * results.data[str(i)][j]), subString[i])
                    meas[subString] = meas[subString] * ((1 - 2 * results.data[str(i)][j]) ** int(subString[i]))
    ...
    for subString in subHamiltonian:
        ...
        expectationValue = total[subString] / shots
