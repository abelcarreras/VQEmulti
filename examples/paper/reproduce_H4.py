...
for label, use_nat_orb in zip(['canonical', 'natural'], [False, True]):

    # run classical calculation
    molecule = run_pyscf(h2_molecule, run_fci=True, nat_orb=use_nat_orb, guess_mix=use_nat_orb, verbose=True)
    ...
    # optimizer
    opt_bfgs = OptimizerParams(method='BFGS', options={'gtol': 1e-4})
    ...
    print('Fidelity: ', result_first['iterations']['fidelity'])
    ...
    print('n_electrons: ', np.trace(density_matrix))
    ...
    fidelity_vactor = 1 - np.array(result_first['iterations']['fidelity'])
    plt.plot(fidelity_vactor, label=label)

plt.title('distance {} '.format(r))
plt.legend()
plt.yscale('log')
plt.ylim(1e-5, 1)
plt.show()