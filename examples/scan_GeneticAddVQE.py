...
for d in np.linspace(0.3, 3, 5):
    ...
    molecule = run_pyscf(h2_molecule, run_fci=True, verbose=True)
    ...
    method = Genetic_Add_Adapt(gradient_threshold=1e-6,
                               diff_threshold=0,
...
plt.plot(np.linspace(0.3, 3, 5), energies_hf, label='HF')
plt.plot(np.linspace(0.3, 3, 5), vqe_energies, label='adaptVQE')
plt.plot(np.linspace(0.3, 3, 5), energies_fullci, label='FullCI')
plt.legend()
...
plt.plot(np.linspace(0.3, 3, 5), diff_fullci, label='adaptVQE')
plt.legend()
