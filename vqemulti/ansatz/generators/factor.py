...
def double_factorized_t2_general(T2_orb, tol=1e-8, max_vecs=None):
    """
    Double-factorized decomposition of a general T2 tensor of shape (norb, norb, norb, norb).
    Amplitudes are not separated into occupied/virtual — works in the full orbital space.

    :param T2_orb: Full T2 tensor with shape (norb, norb, norb, norb)  a_j a_l a_i^ a_k^
    :param tol: Maximum allowed reconstruction error
    :param max_vecs: Maximum number of singular vectors to keep
    :return: diag_coulomb_mats, orbital_rotations : arrays of shape (n_vecs, 2, norb, norb)
    """

    # get dimensions info
    norb = T2_orb.shape[0]
    assert T2_orb.shape == (norb, norb, norb, norb), \
        f"Expected shape ({norb},{norb},{norb},{norb}), got {T2_orb.shape}"

    # first factorization
    # a_j a_l a_i^ a_k^ -> (a_j a_i^) x (a_l a_k^)
    # transpose: (i,j,k,l) -> (j,i,l,k) then reshape to (norb^2 x norb^2)
    t2_mat = T2_orb.transpose(0, 2, 1, 3).reshape(norb * norb, norb * norb)
    outer_eigs, outer_vecs = truncated_eigh(t2_mat, tol=tol, max_vecs=max_vecs)
    n_vecs = len(outer_eigs)
    ...
    for k, outer_vec in enumerate(outer_vecs.T):
        ...
        # reshape outer_vec directly into a (norb x norb) matrix — no embedding needed
        # rows = a_i^ (creator index), cols = a_j (annihilator index)
        mat = outer_vec.reshape(norb, norb)  # a_j a_i^ [full, no embedding needed]

        # decompose mat in m_plus and m_minus such that: 1j*(m_plus - m_minus) == mat - mat.T.conj()
        m_plus  = quadrature(mat, sign=+1)  # Hermitian
        m_minus = quadrature(mat, sign=-1)  # Hermitian

        assert np.allclose(1j * (m_plus - m_minus), mat - mat.T.conj())
        assert np.allclose(m_plus.conj().T,  m_plus),  "m_plus not Hermitian"
        assert np.allclose(m_minus.conj().T, m_minus), "m_minus not Hermitian"

        # second factorization: m = U D U†
        eigs_plus,  orb_rot_plus  = np.linalg.eigh(m_plus)
        eigs_minus, orb_rot_minus = np.linalg.eigh(m_minus)

        # Jastrow matrices J = D ⊗ D  (n_i n_j)
        diag_coulomb_mats[k, 0] = + outer_eigs[k] * np.outer(eigs_plus,  eigs_plus)
        diag_coulomb_mats[k, 1] = - outer_eigs[k] * np.outer(eigs_minus, eigs_minus)

        # Rotation matrices U
        orbital_rotations[k, 0] = orb_rot_plus.conj()
        orbital_rotations[k, 1] = orb_rot_minus.conj()
